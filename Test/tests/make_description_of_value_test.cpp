...
void make_description_of_value_test::from_double()
{
	...
	auto description = make_description(value);
	CPPUNIT_ASSERT( dynamic_cast<pydbc::floating_point_description const *>(description.get()) );
}
...
void make_description_of_value_test::from_string()
{
	...
	field const value(s);
	auto description = make_description(value);
	auto as_string_description = dynamic_cast<pydbc::string_description const *>(description.get());
